...
type Payload = ResourcePayload<Routine>;

export const CreateRoutineForm = ({ handleClose, routine }: Props) => {
  ...
  const {
    ...
    formState: { errors },
    control,
  } = useForm<Payload>({ defaultValues: routine });
  ...
  const submit: SubmitHandler<Payload> = (payload) => {
    ...
    console.log(payload);
    const apiCall = routine
      ? updateResource({ id: routine.id, payload, method: "PUT" })
      ...
      <form onSubmit={handleSubmit(submit)} noValidate>
        <Stack gap={"1em"}>
          ...
          <Flex mt={"1em"} gap={"0.5em"} justifyContent={"flex-end"}>
            ...
            <Button type={"submit"}>{routine ? "Update" : "Create"}</Button>
          </Flex>
        </Stack>
      </form>
    </Box>
