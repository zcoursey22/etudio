...
type Payload = ResourcePayload<Source>;

export const CreateSourceForm = ({ handleClose, source }: Props) => {
  ...
  const {
    ...
    formState: { errors },
    control,
  } = useForm<Payload>({ defaultValues: source });
  ...
  const submit: SubmitHandler<Payload> = (payload) => {
    ...
    const apiCall = source
      ? updateResource({ id: source.id, payload, method: "PUT" })
